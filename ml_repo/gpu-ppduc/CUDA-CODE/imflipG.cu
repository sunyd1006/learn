...
void Vflip(uch *ImgDst, uch *ImgSrc, ui Hpixels, ui Vpixels)
{
	...
	ui BlkPerRow = (Hpixels + ThrPerBlk - 1) / ThrPerBlk;  // ceil, ThrPerBlk -1 相当于对(Hpixels / ThrPerBlk 向上取证
...
void Hflip(uch *ImgDst, uch *ImgSrc, ui Hpixels)
{
  ...
  // [raw, col] 表示一个像素的坐标, 当前thread处理这个像素，并将它防止到ImgDst中特定位置
  // raw递增增长，第0行256个数，第1行是256个数, ... 所以可以计算行号
	ui MYrow = MYbid / BlkPerRow;
  // col也是递增的，第0行[0, 255], 第1行【256, 511], ... 所以可以计算col号
	ui MYcol = MYgtid - MYrow*BlkPerRow*ThrPerBlk;
	if (MYcol >= Hpixels) return;			// col out of range
