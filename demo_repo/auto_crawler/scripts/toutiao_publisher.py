        ...
        try:
                ...
                try:
                    ...
                    if is_contenteditable:
                        await locator.evaluate(
                            "(el, value) => { el.innerHTML = ''; el.innerText = value; el.dispatchEvent(new Event('input', { bubbles: true })); }",
                            content,
                        )
                    else:
            ...
            if not publish_clicked:
                return {
                    "success": False,
                    "title": title,
                    "message": "未找到发布按钮，发布失败",
                }

            await asyncio.sleep(2)

            # 获取发布结果
            publish_url_prefix = "https://mp.toutiao.com/profile_v4/graphic/publish"
            try:
            ...
            current_url = self.page.url
            success = False
            if not current_url.startswith(publish_url_prefix):
                success = True
            else:
                try:
                    if await self.page.locator('text=发布成功').first.is_visible():
                        success = True
                except Exception:
                    pass
    ...
    async def publish_from_file(self, news_file: str) -> list:
        ...
        for i, news in enumerate(news_list):
            ...
            if title and content:
                ...
                delay_seconds = self.config.get("publish", {}).get("delay_seconds", 2)
